...
router.post('/marcas/buscar', function(req, res, cb) {
   let nombre = req.body.nombre;

   Marca.MarcaSelectxNombre(nombre, function(err, result) {
      if(err) {
         return cb(err);
      }
      res.send(result);
...
router.post('/marcas/nuevo/insert', function(req, res, cb) {
   let nombre = req.body.nombre;

   Marca.MarcaInsert(nombre, function(err, result) {
      if(err) {
         return cb(err);
      }
      res.send(result);
